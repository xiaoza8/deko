    ...
    <script>
        $(document).ready(function () {
			...
			$("#table_list").bootstrapTable({
			    ...
			    //获取数据的Servlet地址  
			    url: "${ctx!}/admin/resource/addReourcelist",
			    //表格显示条纹
			    striped: true,
			    //启动分页
			    pagination: true,
			    ...
			    columns: [{
			        ...
			        formatter: function(value,row,index){
			        	if(value == 0)
                    		return '<span class="label label-info">西药品</span>';
                    	else if(value == 1)
                    		return '<span class="label label-primary">医疗机械</span>';
                    	else if(value == 2)
        ...
        function add(){
        	layer.open({
        	      ...
        	      shade: false,
        	      area: ['893px', '600px'],
        	      content: '${ctx!}/admin/resource/addnewResource',
        	      end: function(index){
        	    	  $('#table_list').bootstrapTable("refresh");
       	    	  }
        	    });
